...
const validationSchema = z.object({
  name_ar: z.string().min(2, t('validation.minChars', { count: 2 })).max(100),
  name_en: z.string().min(2, t('validation.minChars', { count: 2 })).max(100),
...
const title = computed(() => (mode.value === 'edit' ? t('categories.editCategory') : t('categories.addCategory')));
</script>
...
<template>
  <UModal :model-value="isOpen" size="md" @update:model-value="isOpen ? null : handleClose()">
    <UCard>
      ...
      <!-- Form Content -->
      <UForm :schema="validationSchema" :state="{ name_ar: nameAr.value, name_en: nameEn.value, parent_id: parentId.value, icon: icon.value, is_active: isActive.value }" class="space-y-4" @submit="handleSubmit">
        <!-- Arabic Name -->
        ...
        <div class="flex justify-end gap-3 pt-4">
          <UButton color="neutral" @click="handleClose" :disabled="isSubmitting.value">
            {{ $t('common.cancel') }}
          </UButton>

<script setup lang="ts">
import { computed } from 'vue';
import { useI18n } from 'vue-i18n';
import { useErrorStore, type Toast } from '../../../stores/errorStore';

const errorStore = useErrorStore();
const { t } = useI18n();

const toasts = computed<Toast[]>(() => errorStore.toasts);

const toastStyles = {
  error: {
    bgClass: 'bg-red-50',
    borderColor: 'shadow-[0px_0px_0px_1px_rgba(239,68,68,0.2)]',
    textColor: 'text-red-900',
    iconColor: 'text-red-600',
    icon: `<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd" /></svg>`,
  },
  warning: {
    bgClass: 'bg-amber-50',
    borderColor: 'shadow-[0px_0px_0px_1px_rgba(217,119,6,0.2)]',
    textColor: 'text-amber-900',
    iconColor: 'text-amber-600',
    icon: `<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd" /></svg>`,
  },
  success: {
    bgClass: 'bg-green-50',
    borderColor: 'shadow-[0px_0px_0px_1px_rgba(34,197,94,0.2)]',
    textColor: 'text-green-900',
    iconColor: 'text-green-600',
    icon: `<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" /></svg>`,
  },
  info: {
    bgClass: 'bg-blue-50',
    borderColor: 'shadow-[0px_0px_0px_1px_rgba(59,130,246,0.2)]',
    textColor: 'text-blue-900',
    iconColor: 'text-blue-600',
    icon: `<svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd" /></svg>`,
  },
};

function handleDismiss(id: string) {
  errorStore.removeToast(id);
}
</script>

<template>
  <Teleport to="body">
    <div v-if="toasts.length > 0" class="fixed inset-0 pointer-events-none">
      <!-- Toast Container - Top Right, RTL Safe -->
      <div class="fixed top-4 end-4 z-50 flex flex-col gap-3 max-w-sm pointer-events-auto">
        <transition-group name="toast" tag="div" class="flex flex-col gap-3">
          <div
            v-for="toastItem in toasts"
            :key="toastItem.id"
            :class="[
              'rounded-lg p-4 flex items-start gap-3',
              toastStyles[toastItem.type].bgClass,
              toastStyles[toastItem.type].borderColor,
            ]"
          >
            <!-- Icon -->
            <div :class="toastStyles[toastItem.type].iconColor" class="flex-shrink-0 mt-0.5">
              <svg
                :class="toastStyles[toastItem.type].iconColor"
                class="w-5 h-5"
                fill="currentColor"
                viewBox="0 0 20 20"
              >
                <path
                  v-if="toastItem.type === 'error'"
                  fill-rule="evenodd"
                  d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z"
                  clip-rule="evenodd"
                />
                <path
                  v-else-if="toastItem.type === 'warning'"
                  fill-rule="evenodd"
                  d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z"
                  clip-rule="evenodd"
                />
                <path
                  v-else-if="toastItem.type === 'success'"
                  fill-rule="evenodd"
                  d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
                  clip-rule="evenodd"
                />
                <path
                  v-else
                  fill-rule="evenodd"
                  d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z"
                  clip-rule="evenodd"
                />
              </svg>
            </div>

            <!-- Content -->
            <div class="flex-1 min-w-0">
              <p :class="toastStyles[toastItem.type].textColor" class="text-sm font-medium">
                {{ toastItem.message }}
              </p>
            </div>

            <!-- Close Button -->
            <button
              :aria-label="t('errors.close', 'Close')"
              class="flex-shrink-0 text-gray-400 hover:text-gray-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 rounded"
              @click="handleDismiss(toastItem.id)"
            >
              <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                <path
                  fill-rule="evenodd"
                  d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                  clip-rule="evenodd"
                />
              </svg>
            </button>
          </div>
        </transition-group>
      </div>
    </div>
  </Teleport>
</template>

<style scoped>
.toast-enter-active,
.toast-leave-active {
  transition: all 300ms ease;
}

.toast-enter-from {
  opacity: 0;
  transform: translateX(30px);
}

.toast-leave-to {
  opacity: 0;
  transform: translateX(30px);
}
</style>

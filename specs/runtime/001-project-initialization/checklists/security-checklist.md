...
- [ ] **CHK043** — Database credentials (DB\*HOST, DB*USER, DB_PASSWORD) in `.env` only  
       \_Priority: **CRITICAL\***  
       \_Note:* Never in `config/database.php` or comments

- [ ] **CHK044** — Mail service credentials (MAIL\*USERNAME, MAIL*PASSWORD) in `.env` only  
       \_Priority: **HIGH\***  
       \_Note:* Deferred: email service stub; credentials prepared for future integration

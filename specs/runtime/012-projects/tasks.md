...
- [ ] T053 [US-ALL] Create `backend/tests/Feature/Api/V1/ProjectControllerTest.php` — test all 6 project endpoints: POST create (valid→201, validation errors→422, non-Customer owner→422, unauthenticated→401, non-Admin→403); GET index (Admin sees all, Customer sees owned, Contractor sees empty; filter by status/type/city; pagination); GET show (authorized→200, unauthorized→403, not found→404, soft-deleted→404 for non-Admin); PUT update (Admin→200, owner→200, CLOSED project→422, non-authorized→403); PUT status (valid transitions→200, invalid transition→422, optimistic lock conflict→409, non-Admin→403); GET timeline (200 with phases, empty phases→200). All responses match error contract.
- [ ] T054 [US6] Create `backend/tests/Feature/Api/V1/ProjectPhaseControllerTest.php` — test all 4 phase endpoints: POST create (valid→201, date outside project range→422, non-Admin→403); GET index (authorized→200 ordered by sort_order, unauthorized→403); PUT update (valid→200, date containment→422); DELETE destroy (valid→200, non-Admin→403). Test phase hard-delete (no soft-delete).
